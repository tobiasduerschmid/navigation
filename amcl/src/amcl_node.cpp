...
{
  ROS_ERROR("ROSTIMER (amcl). Start main: %lu", ros::WallTime::now().toNSec());
  ...
  ROS_ERROR("ROSTIMER (amcl). End main: %lu", ros::WallTime::now().toNSec());
  return(0);
}
...
{
  ROS_ERROR("ROSTIMER (amcl). Start mapReceived: %lu", ros::WallTime::now().toNSec());
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  {
    ROS_ERROR("ROSTIMER (amcl). resampled || force_publication"); 
    ...
    if(max_weight > 0.0)
    {
      ROS_ERROR("ROSTIMER (amcl). max_weight > 0.0"); 
...
{
  ROS_ERROR("ROSTIMER (amcl). Start initialPoseReceived: %lu", ros::WallTime::now().toNSec());
  handleInitialPoseMessage(*msg);
  ROS_ERROR("ROSTIMER (amcl). End initialPoseReceived: %lu", ros::WallTime::now().toNSec());
}
